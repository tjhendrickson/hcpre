...
def setup_conf():
    ...
    # get the template used for getting form the subject dir to the json files
    ...
    config["general"]["json_template"] = json_temp
...
def update_conf(conf_path):
    ...
    if needs_smap:
        ...
        result = pool.map_async(get_series_desc, jsons)
        while not result.ready():
            sleep(.5)
            # perc = float(json_count - result._number_left) / float(json_count)
        ...
        if not series:
            raise ValueError("couldn't find any jsons! please double check your paths and templates...")
